...
const { VALID_REST_OPERATORS } = luna.utils
...
const flattenDeep = (condition) => {
  ...
  const shouldPerformTransformation = (v, k) =>
    _.isObject(v) && !_.isArray(v) && !shouldIgnore(k)
...
const cleanupUnwantedProperties = (condition) => {
  ...
  const shouldClean = (e) =>
    typeof e === 'string'
      ? ops.cleanable.find((o) => e.includes(`.${o}`))
      : undefined
  ...
  return _.reduce(
    ...
    (acc, value, key) => {
      ...
      return {
        ...acc,
        [newKey]: _.isArray(value)
          ? value.map(cleanupUnwantedProperties)
          : value
      }
    },
    {}
