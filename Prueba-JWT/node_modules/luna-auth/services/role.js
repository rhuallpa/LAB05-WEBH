...
const sortPermissionProperties = (permission) => {
  return Object.entries(permission.properties).reduce(
    (acc, [name, value]) =>
      permissionDomain.setProperty(name, sortDeep(value), acc),
    permission
  )
}
...
const create = async (attributes) => {
  ...
  if (alreadyExists) {
    throw luna.errors.badRequest('ValidationError', {
      name: [
  ...
  const autoGeneratedCode = `${_.kebabCase(
    attributes.name
  )}-${generateTimestampCode()}`
  ...
  return luna.query('role', 'admin').create(rolesWithCode)
}
...
const findOne = (params = {}, populate = []) => {
  return luna.query('role', 'admin').findOne(params, populate)
}
...
const findOneWithUsersCount = async (params = {}, populate = []) => {
  const role = await luna.query('role', 'admin').findOne(params, populate)
...
const find = (params = {}, populate = []) => {
  return luna.query('role', 'admin').find(params, populate)
}
...
const findAllWithUsersCount = async (populate = []) => {
  const roles = await luna.query('role', 'admin').find({ _limit: -1 }, populate)
...
const update = async (params, attributes) => {
  ...
  return luna.query('role', 'admin').update(params, sanitizedAttributes)
}
...
const exists = async (params) => {
  const foundCount = await luna.query('role', 'admin').count(params)
...
const count = async (params = {}) => {
  return luna.query('role', 'admin').count(params)
}
...
const deleteByIds = async (ids = []) => {
  ...
  let deletedRoles = await luna.query('role', 'admin').delete({ id_in: ids })
...
const getUsersCount = async (roleId) => {
  return luna.query('user', 'auth').count({ roles: [roleId] })
}
...
const getSuperAdminWithUsersCount = () =>
  findOneWithUsersCount({ code: SUPER_ADMIN_CODE })
...
const displayWarningIfNoSuperAdmin = async () => {
  ...
  if (!superAdminRole) {
    luna.log.warn("Your application doesn't have a super admin role.")
  } else if (someUsersExists && superAdminRole.usersCount === 0) {
    luna.log.warn("Your application doesn't have a super admin user.")
  }
}
...
const assignPermissions = async (roleId, permissions = []) => {
  try {
    ...
    throw luna.errors.badRequest('ValidationError', err)
  }
  ...
  if (permissionsToAdd.length > 0) {
    ...
    permissionsToReturn.push(
      ...createdPermissions.map((p) => ({ ...p, role: p.role.id }))
    )
  }
...
const resetSuperAdminPermissions = async () => {
  ...
  const permissions =
    getService('content-type').getPermissionsWithNestedFields(
      contentTypesActions
    )
    ...
    if (isArray(subjects)) {
      acc.push(
        ...subjects.map((subject) =>
          permissionDomain.create({ action: actionId, subject })
        )
      )
    } else {
  ...
  const transformedPermissions =
    await hooks.willResetSuperAdminPermissions.call(permissions)
