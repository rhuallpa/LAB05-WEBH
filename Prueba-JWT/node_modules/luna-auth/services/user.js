...
const { stringIncludes } = luna.utils
const { createUser, hasSuperAdminRole } = require('../domain/user')
const {
  password: passwordValidator
} = require('../validation/common-validators')
...
const create = async (attributes) => {
  ...
  if (_.has(attributes, 'password')) {
    userInfo.password = await luna.services.auth.hashPassword(
      attributes.password
    )
  }
  userInfo.registrationToken = luna.services.token.createToken()
  const user = createUser(userInfo)
  const createdUser = await luna.query('user', 'auth').create(user)
...
const updateById = async (id, attributes) => {
  ...
  if (_.has(attributes, 'roles')) {
    ...
    const superAdminRole =
      await luna.services.role.getSuperAdminWithUsersCount()
  ...
  if (_.has(attributes, 'password')) {
    const hashedPassword = await luna.services.auth.hashPassword(
      attributes.password
    )

    return luna.query('user', 'auth').update(
      { id },
      {
        ...attributes,
  ...
  return luna.query('user', 'auth').update({ id }, attributes)
}
...
const isLastSuperAdminUser = async (userId) => {
  ...
  const superAdminRole = await luna.services.role.getSuperAdminWithUsersCount()
...
const exists = async (attributes = {}) => {
  return (await luna.query('user', 'auth').count(attributes)) > 0
}
...
const findRegistrationInfo = async (registrationToken) => {
  const user = await luna.query('user', 'auth').findOne({ registrationToken })
...
const register = async ({ registrationToken, userInfo }) => {
  const matchingUser = await luna
    .query('user', 'auth')
    .findOne({ registrationToken })

  if (!matchingUser) {
    throw luna.errors.badRequest('Invalid registration info')
  }
...
const findOne = async (params, populate) => {
  return luna.query('user', 'auth').findOne(params, populate)
}
...
const findPage = async (query) => {
  return luna.query('user', 'auth').findPage(query)
}
...
const searchPage = async (query) => {
  return luna.query('user', 'auth').searchPage(query)
}
...
const deleteById = async (id) => {
  ...
  return luna.query('user', 'auth').delete({ id })
}
...
const deleteByIds = async (ids) => {
  // Check at least one super admin remains
  const superAdminRole = await luna.services.role.getSuperAdminWithUsersCount()
  const nbOfSuperAdminToDelete = await luna
    .query('user', 'auth')
  ...
  return luna.query('user', 'auth').delete({ id_in: ids })
}
...
const countUsersWithoutRole = async () => {
  const userModel = luna.query('user', 'auth').model
  let count

  if (userModel.orm === 'bookshelf') {
    count = await luna.query('user', 'auth').count({ roles_null: true })
  } else if (userModel.orm === 'mongoose') {
    count = await luna.query('user', 'auth').model.countDocuments({
      $or: [{ roles: { $exists: false } }, { roles: { $size: 0 } }]
    })
  } else {
    const allRoles = await luna.query('role', 'auth').find({ _limit: -1 })
    count = await luna.query('user', 'auth').count({
...
const count = async (params = {}) => {
  return luna.query('user', 'auth').count(params)
}
...
const assignARoleToAll = async (roleId) => {
  ...
  if (userModel.orm === 'bookshelf') {
    const assocTable = userModel.associations.find(
      (a) => a.alias === 'roles'
    ).tableCollectionName
    const userTable = userModel.collectionName
    const knex = luna.connections[userModel.connection]
    ...
    if (usersIds.length > 0) {
      const newRelations = usersIds.map((userId) => ({
        user_id: userId,
        role_id: roleId
      }))
    ...
    await luna.query('user', 'auth').model.updateMany({}, { roles: [roleId] })
  }
}
...
const displayWarningIfUsersDontHaveRole = async () => {
  ...
  if (count > 0) {
    luna.log.warn(`Some users (${count}) don't have any role.`)
  }
}

const migrateUsers = async () => {
  const someRolesExist = await luna.services.role.exists()
  if (someRolesExist) {
    return
  }
