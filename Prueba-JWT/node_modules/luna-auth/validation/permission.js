...
const { yup, formatYupErrors } = luna.utils
...
const checkPermissionsAreBound = (role) =>
  function (permissions) {
    ...
    for (const [subject, perms] of Object.entries(permsBySubject)) {
      ...
      const permsBoundByFields = perms.filter((p) =>
        BOUND_ACTIONS_FOR_FIELDS.includes(p.action)
      )
      const everyActionsHaveSameFields = _.every(
        permsBoundByFields,
        haveSameFieldsAsOtherActions
      )
...
const noPublishPermissionForAuthorRole = (role) =>
  function (permissions) {
    ...
    const hasPublishPermission = permissions.some(
      (perm) => perm.action === PUBLISH_ACTION
    )
      ...
      !existingActions.some(
        (action) =>
          ...
          (action.section !== 'contentTypes' ||
            action.subjects.includes(permission.subject))
      )
  )
